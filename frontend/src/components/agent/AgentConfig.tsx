...
import type { AgentConfigSchema, AgentDefinition } from "@/types/agent";
...
export function AgentConfig({ schema }: AgentConfigProps) {
  const formValues = useAgentStore((s) => s.formValues);
  const setFormValue = useAgentStore((s) => s.setFormValue);
  const executeAgent = useAgentStore((s) => s.executeAgent);
  const executing = useAgentStore((s) => s.executing);
  const [validationErrors, setValidationErrors] = useState<Record<string, string>>({});
  const [uploadingField, setUploadingField] = useState<string | null>(null);
  const [uploadedFiles, setUploadedFiles] = useState<Record<string, string>>({});
  const fileRefs = useRef<Record<string, HTMLInputElement | null>>({});
  const moduleAccent = theme.colors.modules.agent;
  const isBatch = schema.agent_id === "batch";
  ...
  const handleFileUpload = async (fieldName: string, file: File) => {
    ...
    try {
      ...
      fd.append("file", file);
      const resp = await fetch(`${API_BASE}/api/agent/batch/upload-companies`, {
        method: "POST",
        body: fd,
      });
  ...
  const applyBatchExample = (example: string) => {
    setFormValue("query", example);
        ...
        <div className="mb-4 rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 space-y-2.5">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {activeBatchExamples.map((example) => (
              <button
                key={example}
                type="button"
                onClick={() => applyBatchExample(example)}
          ...
          {scenario === "export" && (
            <p className="text-[11px] text-white/35">
              无需手动点选导出字段，直接在问句中描述字段名称即可（如股东、法院公告、AI方向、营收、净利润）。
            </p>
          )}
        </div>
      )}
        ...
        {schema.fields.map((field) => {
          ...
          <div key={field.name}>
                ...
                {field.options?.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setFormValue(field.name, opt.value);
                        if (validationErrors[field.name]) {
                          setValidationErrors((prev) => {
                            const next = { ...prev };
                            delete next[field.name];
            ...
            {field.type === "multiselect" && (
              <div className="flex flex-wrap gap-2">
                {field.options?.map((opt) => {
                  const currentArr = Array.isArray(formValues[field.name])
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setFormValue(field.name, newArr);
                        if (validationErrors[field.name]) {
                          setValidationErrors((prev) => {
                            const next = { ...prev };
                            delete next[field.name];
                ...
                >
                  {uploadingField === field.name ? (
                    ...
                    <div className="flex flex-col items-center gap-1.5">
                      ...
                      <p className="text-[12px]" style={{ color: moduleAccent }}>
                ...
                {uploadedCompanyCount > 0 && (
                  <div className="mt-1.5 space-y-1">
                    <p className="text-[11px] text-white/40">
                      已解析企业 {uploadedCompanyCount} 家，将与文本名单自动合并去重。
                    </p>
                    <p className="text-[11px] text-white/30 truncate">
                      示例：{(formValues.company_names as string[]).slice(0, 6).join("、")}
                      {uploadedCompanyCount > 6 ? " …" : ""}
                    </p>
                  </div>
                )}
              </div>
