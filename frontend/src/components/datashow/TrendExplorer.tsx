...
import { TrendQueryInput } from "./TrendQueryInput";
...
export function TrendExplorer() {
  const currentTaskId = useTrendExplorerStore((s) => s.currentTaskId);
  const taskStatus = useTrendExplorerStore((s) => s.taskStatus);
  const result = useTrendExplorerStore((s) => s.result);
  const error = useTrendExplorerStore((s) => s.error);
  const loadingResult = useTrendExplorerStore((s) => s.loadingResult);
  const queryHistory = useTrendExplorerStore((s) => s.queryHistory);
  const deepDive = useTrendExplorerStore((s) => s.deepDive);
  const submitQuery = useTrendExplorerStore((s) => s.submitQuery);
  const cancelExecution = useTrendExplorerStore((s) => s.cancelExecution);
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <TrendProgress onStop={cancelExecution} />
      ...
      {!showResult && !isRunning && !loadingResult && !error && (
        <div className="flex flex-col items-center justify-center py-16 animate-fadeIn">
          ...
          <p className="text-[13px] text-white/28 mb-6 text-center max-w-[360px]">
            通过自然语言查询企业趋势数据，AI 将自动搜索、分析并生成多维度趋势可视化
          </p>

          {/* Quick start suggestions */}
          <div className="flex flex-wrap justify-center gap-2">
            {["同花顺近一年风险趋势", "比亚迪 vs 宁德时代", "新能源行业趋势"].map((s) => (
              <button
                key={s}
...
function entityColor(type: string, alpha: number): string {
  const colors: Record<string, string> = {
    ...
    industry: `rgba(167,139,250,${alpha})`,
