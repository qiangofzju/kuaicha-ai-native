"""Skill creator should bind generated skill to workspace session."""

import asyncio
import shutil
import unittest

from app.agents.registry import register_all_agents
from app.services.skill_service import skill_service
from app.services.workspace.workspace_session import workspace_session_service


async def _wait_run_done(run_id: str, timeout_sec: float = 25.0):
    steps = int(timeout_sec / 0.05)
    for _ in range(steps):
        status = skill_service.get_run_status(run_id)
        if status and status.get("status") in {"completed", "failed", "cancelled"}:
            break
        await asyncio.sleep(0.05)


class SkillCreatorWorkspaceBindingTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        register_all_agents()

    def test_creator_binds_workspace_and_writes_files(self):
        async def _run():
            session = workspace_session_service.create_session(purpose="skill_create", seed_prompt="")
            for _ in range(80):
                record = workspace_session_service.get(session.workspace_id)
                if record and record.status == "ready":
                    break
                await asyncio.sleep(0.05)

            run = await skill_service.create_run(
                skill_id="skill-creator",
                input_payload={"query": "创建workspace-binding技能用于验证"},
                context={
                    "source": "test",
                    "workspace_id": session.workspace_id,
                    "session_id": session.session_id,
                },
            )
            run_id = str(run["run_id"])
            await _wait_run_done(run_id)
            result = skill_service.get_run_result(run_id)
            self.assertIsNotNone(result)
            assert result is not None

            created_skill_id = str(result.get("created_skill_id", ""))
            self.assertTrue(created_skill_id)

            record = workspace_session_service.get(session.workspace_id)
            self.assertIsNotNone(record)
            assert record is not None
            self.assertTrue(record.display_root.endswith(f"/{created_skill_id}"))

            tree = workspace_session_service.list_tree(session.workspace_id)
            flat_names = {node.get("name") for node in tree.get("nodes", [])}
            self.assertIn("manifest.json", flat_names)

            shutil.rmtree(record.real_root, ignore_errors=True)
            skill_service.refresh_catalog()

        asyncio.run(_run())


if __name__ == "__main__":
    unittest.main()
